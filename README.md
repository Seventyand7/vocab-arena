...
規則裡的 `allowedUids()` 還多鎖了一層：因為網站是公開的，只有白名單內的帳號能存取資料，其他人就算用自己的 Google 帳號登入也讀寫不了任何東西。

### 開通新的使用者

1. 請對方到網站上用 Google 登入一次。因為還沒有權限，畫面會顯示**他自己的 UID** 並自動登出。
2. 把那串 UID 加進 `firestore.rules` 的 `allowedUids()` 清單。
3. 重新發布規則。

要停用某個人，把他那行從清單刪掉再發布即可。

> 知道 UID 不等於拿到存取權 —— 對方必須真的用擁有那個 UID 的 Google 帳號登入，UID 只是用來指名，不是密碼。
>
> 每個人的單字庫各自存在自己的 `users/{uid}` 底下，**彼此看不到對方的資料**，也不會共用。

自己的 UID 登入後在「設定 → ACCOUNT」可以直接複製。
