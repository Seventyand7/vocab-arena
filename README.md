...
規則裡的 `ownerUid()` 還多鎖了一層：因為網站是公開的，只有你自己的帳號能存取資料，別人就算用自己的 Google 帳號登入也讀寫不了任何東西（前端會提示並自動登出）。**發布前記得把 `REPLACE_WITH_YOUR_UID` 換成你自己的 UID**，登入後在「設定 → ACCOUNT」可以直接複製，換之前先別發布，否則你自己也會被鎖在外面。
